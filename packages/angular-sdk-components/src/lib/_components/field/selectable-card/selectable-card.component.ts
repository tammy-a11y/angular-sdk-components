...
export class SelectableCardComponent implements OnInit, OnDestroy {
  ...
  updateSelf(): void {
    ...
    if (this.type === 'radio') {
      ...
      cardDataSource = this.commonProps?.datasource?.source || [];
    } else if (this.type === 'checkbox') {
      ({ image, recordKey, cardLabel } = this.getCheckboxProps(imagePosition, imageSize, showImageDescription));
      this.selectionList = this.configProps$.selectionList;
      this.selectedvalues = this.configProps$.readonlyContextList;
      this.showNoValue = this.readOnly && this.selectedvalues.length === 0;
