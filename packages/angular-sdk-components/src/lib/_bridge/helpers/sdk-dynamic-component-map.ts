 ...
 * IMPORTANT: Keep this file side-effect free except for ErrorBoundary fallback import.
 */
...
const componentClassCache: Record<string, Type<any>> = {};
 ...
 * Generated initially from sdk-pega-component-map.ts. You can script-regenerate if new components are added.
 * Infra
 * (Entries between these markers are managed by scripts/generate-dynamic-loader-map.mjs)
 */
...
/**
 * Optionally prefetches critical components to improve shell load performance.
 *
 * This function can be called by the consumer after bootstrap to eagerly load
 * a set of components that are essential for the initial application shell.
 *
 * @param names Array of component names to prefetch. Defaults to ['RootContainer', 'ViewContainer'].
 */
export function prefetchCriticalComponents(names: string[] = ['RootContainer', 'ViewContainer']) {
  names.forEach(name => {
    getComponentClassAsync(name);
  });
}

// Expose for potential debugging / metrics
export function __getDynamicComponentCacheKeys() {
  return Object.keys(componentClassCache);
}
